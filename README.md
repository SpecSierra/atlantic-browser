...
| Qt | **5.6.3**, C++17, qmake |
| WPE WebKit | **2.54.0** via `wpewebkit-2.0` / `wpe-1.0` |
| Package | `atlantic-browser` **1.4.0** (set by `ATLANTIC_BROWSER_VERSION` in atlantic-engine `versions.env`; `rpm/sailfish-browser.spec` is the unused upstream spec) |
...
| Engine (external) | `WPEQtView` ← WPE WebKit 2.54.0, `libWPEWebKit-2.0`, `libwpe-1.0` |
