    ...
    , m_security(new WPESecurityInfo(this))
{
    ...
    connect(this, &WPEQtView::webViewCreated, this, [this]() {
        if (WebKitWebView* wv = webView()) {
            ...
            // paint-holding keeps the old frame up until it paints.
            //
            // Do NOT try to identify the destination here to skip reloads.
            // webkit_web_view_get_uri() still returns the OUTGOING url at this
            // point — WebKitWebView.cpp only syncs activeURI to the page load
            // state at WEBKIT_LOAD_COMMITTED, and blocks active-URI changes
            // before STARTED (isActiveURIChangeBlocked). An earlier revision
            // compared it against m_lastCommittedUrl to skip same-document
            // loads; the two are always equal here, so it skipped everything
            // and the capture never ran. Re-capturing on a reload is harmless:
            // same key, same file, fresher pixels.
            g_signal_connect(
                wv, "load-changed",
                G_CALLBACK(+[](WebKitWebView*, WebKitLoadEvent event, gpointer userData) {
