...
// byte leaves. This reports the link under the finger at touch-DOWN so the UI
// process can open the connection during the ~80-150 ms the user spends
// completing the tap. Nothing is fetched — see wpe_sfos_preconnect().
//
// Deliberately touchstart and not a hover/mouseover: there is no hover on this
// device, and a pointerdown-time hit test is the earliest signal that is also
// a real prediction (the finger is already on the link) rather than a guess.
//
// Single-finger only, so a pinch or a two-finger scroll never speculates. The
// script dedupes consecutive identical origins; WPEWebPage::preconnect() holds
// the authoritative per-origin rate limit.
static const char* const kPreconnectBridge = R"JS(
(function() {
    if (window.__wpePreconnectBridgeInstalled) return;
    window.__wpePreconnectBridgeInstalled = true;
    ...
    function onTouchStart(e) {
        try {
            ...
            var a = anchorAt(e.touches[0].target);
            if (!a) return;

            // a.href is already absolute and resolved against the base URI.
            var href = a.href;
            if (href.lastIndexOf('http:', 0) !== 0 && href.lastIndexOf('https:', 0) !== 0)
                return;

            var now = Date.now();
            if (href === lastUrl && (now - lastAt) < 2000) return;
            lastUrl = href;
            lastAt = now;

            window.webkit.messageHandlers.preconnectBridge.postMessage({ url: href });
        } catch (err) {}
    }
...
// Performance interventions (lever 5). The adblock stack already ships a
// weekly-refreshed rule pipeline, a pre-paint user stylesheet and document-start
// scriptlets — all of it aimed at ADS. This reuses the same shape for the other
// half of what makes a heavy news site slow on this hardware: work the page asks
// for that the user never sees.
//
// Four interventions, each individually switchable per site by the rule file
// (see data/perf-interventions.json). All of them DELAY work; none of them drop
// it, because dropping is the adblocker's job and it has a filter list behind it.
//
//   deferScripts  Third-party tag managers / analytics / recommendation widgets
//                 are fetched and executed on the main thread while the article
//                 the user asked for is still laying out. Deferral is limited to
//                 scripts the page creates from JS (document.createElement), for
//                 a load-bearing reason: by the time a MutationObserver sees a
//                 PARSER-inserted <script src>, "prepare a script" has already
//                 run and neither removing the node nor rewriting its type can
//                 stop it. WebKit has no beforescriptexecute. The dynamic path
//                 is both interceptable and where the loaders that matter live
//                 (GTM, and every analytics snippet that injects its own tag).
//                 Released on the FIRST of: a real user gesture, or idleMs after
//                 load — so anything the user can actually see unblocks the
//                 moment they touch the page.
//
//   lazyImages    loading="lazy" + decoding="async" on images that declare
//                 neither. The first `eagerImages` images in document order are
//                 left eager so the LCP candidate is never delayed.
//
//   lazyFrames    loading="lazy" on iframes that declare nothing: social embeds,
//                 players and surviving ad frames each cost a document, a
//                 network stack and their own compositing layers.
//
//   pauseAnims    Infinite CSS animations outside the viewport are paused via
//                 the Web Animations API (document.getAnimations() -> pause /
//                 play driven by an IntersectionObserver). Spinners and marquees
//                 far down a page otherwise keep the compositor and the style
//                 system awake for content nobody is looking at.
//
// The rule file is injected as the argument, so per-site behaviour is decided in
// JS from location.hostname and the C++ side stays a loader.
static const char* const kPerfInterventions = R"JS(
(function(cfg) {
    if (window.__atlPerfInterventionsInstalled) return;
    window.__atlPerfInterventionsInstalled = true;
    if (!cfg || !cfg.defaults) return;

    // ---- rule resolution -------------------------------------------------
    var host = '';
    try { host = String(location.hostname || '').toLowerCase(); } catch (e) { return; }

    function hostMatches(pattern) {
        if (!pattern) return false;
        var p = String(pattern).toLowerCase();
        return host === p || (host.length > p.length && host.slice(-(p.length + 1)) === '.' + p);
    }

    var rule = {};
    for (var k in cfg.defaults) rule[k] = cfg.defaults[k];

    var deferPatterns = (cfg.deferPatterns || []).slice();
    var siteCss = '';

    var sites = cfg.sites || [];
    for (var i = 0; i < sites.length; i++) {
        var site = sites[i];
        if (!site || !hostMatches(site.match)) continue;
        var disable = site.disable || [];
        for (var d = 0; d < disable.length; d++) rule[disable[d]] = false;
        var enable = site.enable || [];
        for (var e2 = 0; e2 < enable.length; e2++) rule[enable[e2]] = true;
        if (typeof site.eagerImages === 'number') rule.eagerImages = site.eagerImages;
        if (site.defer) deferPatterns = deferPatterns.concat(site.defer);
        if (site.css) siteCss += site.css;
    }

    // ---- 1. deferred third-party scripts ---------------------------------
    // Held scripts are released on the first user gesture or after the idle
    // deadline, whichever comes first. Never dropped.
    var held = [];
    var released = false;
    var restoreCreateElement = null;

    function release() {
        if (released) return;
        released = true;
        // Un-hook createElement first: the wrapper costs a JS frame on every
        // element a page builds, and it has nothing left to do.
        if (restoreCreateElement) {
            try { restoreCreateElement(); } catch (e) {}
    ...
    function shouldDefer(url) {
        if (!url) return false;
        var u = String(url);
        for (var i = 0; i < deferPatterns.length; i++) {
            if (u.indexOf(deferPatterns[i]) !== -1) return true;
        }
        return false;
    }

    if (rule.deferScripts && deferPatterns.length) {
        try {
            var nativeCreateElement = document.createElement;
            var nativeCreate = nativeCreateElement.bind(document);
            var srcDesc = Object.getOwnPropertyDescriptor(HTMLScriptElement.prototype, 'src');
            var nativeSetAttribute = Element.prototype.setAttribute;

            restoreCreateElement = function() { document.createElement = nativeCreateElement; };
            document.createElement = function(name) {
                var el = nativeCreate.apply(null, arguments);
                try {
                    if (released || !name || String(name).toLowerCase() !== 'script')
                        return el;

                    // Intercept BOTH ways a src can arrive; hold the value and
                    // replay it verbatim at release time, so the element the
                    // page keeps a reference to is the element that eventually
                    // loads (its onload/onerror handlers still fire).
                    var pending = null;
                    var apply = function() {
                    ...
                    Object.defineProperty(el, 'src', {
                        configurable: true,
                        enumerable: true,
                        get: function() { return pending !== null ? pending : srcDesc.get.call(el); },
                        set: function(value) {
                    ...
                    el.setAttribute = function(attr, value) {
                        if (!released && String(attr).toLowerCase() === 'src' && shouldDefer(value)) {
                            pending = value;
                            held.push(apply);
                            return;
                        }
                        return nativeSetAttribute.call(el, attr, value);
                    };
                } catch (e) {}
                return el;
            };

            var gestures = ['touchstart', 'mousedown', 'keydown', 'wheel'];
            for (var g = 0; g < gestures.length; g++)
                window.addEventListener(gestures[g], release, { passive: true, capture: true, once: true });

            var idleMs = (cfg.deferUntil && cfg.deferUntil.idleMs) || 2500;
            window.addEventListener('load', function() { setTimeout(release, idleMs); }, { once: true });
            // Backstop: a page that never fires load must not hold them forever.
            setTimeout(release, idleMs + 8000);
        } catch (e) {}
    }

    // ---- 2/3. lazy images and frames -------------------------------------
    var eagerBudget = typeof rule.eagerImages === 'number' ? rule.eagerImages : 3;

    function tagMedia(node) {
        try {
            if (node.tagName === 'IMG') {
                if (!node.hasAttribute('decoding')) node.setAttribute('decoding', 'async');
                if (rule.lazyImages && !node.hasAttribute('loading')) {
                    if (eagerBudget > 0) eagerBudget--;
                    else node.setAttribute('loading', 'lazy');
                }
            } else if (node.tagName === 'IFRAME') {
                if (rule.lazyFrames && !node.hasAttribute('loading'))
                    node.setAttribute('loading', 'lazy');
            }
        } catch (e) {}
    }

    if (rule.lazyImages || rule.lazyFrames) {
        try {
            var mediaObserver = new MutationObserver(function(records) {
                for (var i = 0; i < records.length; i++) {
                    ...
                    for (var j = 0; j < added.length; j++) {
                        ...
                        tagMedia(node);
                        if (node.querySelectorAll) {
                            var nested = node.querySelectorAll('img, iframe');
                            for (var n = 0; n < nested.length; n++) tagMedia(nested[n]);
                        }
                    }
                }
            });
            ...
            window.addEventListener('load', function() {
                setTimeout(function() { try { mediaObserver.disconnect(); } catch (e) {} }, 5000);
            }, { once: true });
        } catch (e) {}
    }

    // ---- 4. pause offscreen infinite animations ---------------------------
    if (rule.pauseAnims) {
        try {
            var seen = new WeakSet();
            var byTarget = new WeakMap();

            var intersection = new IntersectionObserver(function(entries) {
                for (var i = 0; i < entries.length; i++) {
                    var list = byTarget.get(entries[i].target);
                    if (!list) continue;
                    for (var a = 0; a < list.length; a++) {
                        try {
                            if (entries[i].isIntersecting) list[a].play();
                            else list[a].pause();
                        } catch (e) {}
                    }
                }
            ...
            var scheduleScan = function() {
                if (scanTimer) return;
                scanTimer = setTimeout(function() { scanTimer = 0; scan(); }, 500);
            };

            window.addEventListener('load', scheduleScan, { once: true });
            document.addEventListener('DOMContentLoaded', scheduleScan, { once: true });
            // Late-arriving animations (lazy widgets); bounded, not a live hook.
            var rescans = 0;
            var rescanTimer = setInterval(function() {
                if (++rescans > 6) { clearInterval(rescanTimer); return; }
                scan();
            }, 3000);
        } catch (e) {}
    }

    // ---- per-site lite stylesheet ----------------------------------------
    if (siteCss) {
        try {
